...
## 1.2 — сцена сжимается на ≤1280×720 после min-height:0 (2026-05-22)

**Файл:** `src/styles/archimedes-experiment.css` `.archimedes-stage-area`.

### Контекст
В рамках паритета kit-1 (опыты к виду 1.1) `min-height: 660px` заменён на
`min-height: 0` — чтобы 4 уголка канваса и журнал всегда были в кадре (раньше
жёсткий пол выталкивал их за вьюпорт). Проверено: на **≥1440×810** (и на экране
пользователя ~1536) полный стенд (динамометр+цилиндр№3+стакан, цилиндр в воде)
лежит корректно — цилиндр внутри стакана (cyl_bottom на ~23px ВЫШЕ дна стакана),
все уголки + журнал видны.

### Симптом (только ≤1280×720)
На узком вьюпорте stage-трек ≈431px < ~560px, нужных сцене. Цилиндр висит на
фиксированной (JS) глубине от крюка динамометра, а стакан привязан
`bottom: 200px` от низа сцены → на коротком треке стакан «уезжает» вверх, и
**нижняя кромка цилиндра оказывается ниже дна стакана (~100px)**.

### Почему не починено «по-быстрому»
Это НЕ CSS-`--w-size`-правка. Позиции 1.2 захардкожены в px под высоту сцены ~656:
`.ar-mount--beaker { bottom: 200px }`, JS `baseTop≈231`, `DIP_Y_OFFSET_PX=312` в
`ArchimedesExperiment.ts`. Респонсив для 1.2 = рекалибровка dip-геометрии (риск
регресса физики погружения). Эталоны 1.1/1.3 не страдают: у них сцена — flex и
приборы мельче.

### Когда чинить (отдельная задача)
Перевести сцену 1.2 на stage-относительное позиционирование (как `.av-rig` в 1.3,
flex-колонка) + рекалибровать dip-офсеты под любой vp. Обязательно прогнать все
dip-geometry e2e/unit + Playwright на 1280/1366/1440. Решение пользователя
2026-05-22: ship min-height:0 сейчас (чинит реальный экран), refactor — позже.

### Не блокирует
- Экран пользователя (~1536) и все ≥1440 — корректны.
- Журнал/степпер паритет (T1-T4) — независимы, готовы.
