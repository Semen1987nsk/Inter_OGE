/**
 * <lab-weight mass="100" number="1" type="standard|fine|mystery" color="#XXX">
 *
 ...
 * белая наклейка с НОМЕРОМ груза (не массой) по центру.
 *
 * По ФИПИ ОГЭ-2026 грузы в комплекте №2 пронумерованы — №1/№2/№3 —
 * а массу ученик ОПРЕДЕЛЯЕТ САМ через взвешивание на динамометре. Это
 * часть учебной методики: «знать массу нельзя, нужно измерить». Поэтому
 * на цилиндре грузa показываем номер, не «100 г».
 *
 * Атрибуты:
 *   mass    — масса (граммы) для физики, не отображается на корпусе
 *   number  — номер груза (1, 2, 3...) для отображения. Если не задан —
 *             SVG-наклейка пустая (только корпус).
 *   type    — 'standard' | 'fine' | 'mystery' (mystery → отображается «?»)
 *   color   — кастомный цвет для type='mystery'
 *
...
export class LabWeight extends HTMLElement {
  static observedAttributes = ['mass', 'type', 'color', 'number'];
  ...
  attributeChangedCallback(name: string): void {
    if (name === 'mass' || name === 'type' || name === 'number') this.#updateLabel();
  ...
  #updateLabel(): void {
    if (this.type === 'mystery') {
      ...
      // По ФИПИ: на грузе номер, не масса. Ученик взвешивает сам.
      this.#svgLabel.textContent = this.number;
    } else {
      this.#svgLabel.textContent = '';
    }
    this.setAttribute(
      ...
      this.type === 'mystery'
        ...
        : this.number !== null
          ? `Груз №${this.number} — взвесьте на динамометре. Нажмите Enter чтобы взять.`
          : 'Груз, нажмите Enter чтобы взять',
    );
  }
