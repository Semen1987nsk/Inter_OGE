...
export class HintEngine {
  ...
  #hintForState(s: FrictionSetupState): string {
    ...
    const isWork = s.activeTask === 'B-work';
    // Динамометр прицеплен — фаза измерения
    switch (s.measurementStep) {
      case 'awaiting-pull':
        return isWork
          ? 'Тяните динамометр вправо до скольжения. Брусок поедет — следите за индикатором пути «s».'
          : 'Тяните за корпус динамометра вправо — равномерно, до начала скольжения.';
      case 'pulling-static':
        return 'Брусок ещё держит покой — увеличивайте силу плавно.';
      case 'sliding':
        return isWork
          ? 'Скольжение пошло! Двигайте равномерно — путь «s» растёт. Зафиксируйте F и s.'
          : 'Скольжение пошло! Удерживайте равномерное движение и зафиксируйте показание.';
      case 'ready-to-record':
        return isWork
          ? 'Спишите F тр с динамометра и путь s с индикатора, затем «Записать в журнал».'
          : 'Хорошо. Нажмите «Записать в журнал» для фиксации измерения.';
      case 'recorded': {
        ...
        if (isWork) {
          return 'Запись сохранена. Работа A = F·s посчитана. Можно повторить или «Сбросить».';
        }
