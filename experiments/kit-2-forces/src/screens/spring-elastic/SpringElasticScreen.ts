/**
 * SpringElasticScreen — экран «Сила упругости / F(Δl)» (опыт 2.7 по ФИПИ).
 *
 ...
 *   2.7: ученик строит график F(Δl) и убеждается в его линейности (закон Гука).
 *
...
export class SpringElasticScreen implements IScreen {
  readonly meta: ScreenMeta = {
    id: 'spring-elastic',
    label: 'Закон Гука',
    // По ФИПИ ОГЭ-2026 «исследование F_упр(Δl)» = опыт 2.7 (КОДИФ §1.29).
    // Числа 2.2–2.6 заняты вариантами трения; 2.7 — первый незанятый из серии «силы».
    kicker: 'Опыт 2.7',
    icon: 'force',
  ...
  mount(host: HTMLElement): void {
    ...
    // достигается через kit-header (kicker «Опыт 2.7»).
    const eyebrow = host.querySelector('.experiment-eyebrow');
    if (eyebrow) eyebrow.textContent = 'Опыт 2.7';
