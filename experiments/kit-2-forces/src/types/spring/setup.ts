...
export type AttachKind = 'spring' | 'dynamometer' | 'weight' | 'disc';
 ...
 *  composite-load: масса вычисляется динамически (10 + sum(discs)); запись здесь
 *  носит технический характер (минимум 10 г = пустая штанга). */
export const WEIGHT_CONFIG: Record<string, { mass: number; equipmentId: EquipmentId }> = {
  ...
  'composite-load': { mass: 10, equipmentId: 'composite-load' },
};

/** Считает массу собранного узла по списку надетых дисков (10/20/50 г). */
export function getCompositeLoadMass(discs: ReadonlyArray<number>): number {
  return 10 + discs.reduce((acc, m) => acc + m, 0);
}
