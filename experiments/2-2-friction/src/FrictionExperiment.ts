...
// Локальный `experiments/2-2-friction/src/controller/DragController.ts`
// остаётся как back-compat для тестов; активно больше не используется.
import {
  DragController,
  type SnapZone,
...
import { FRICTION_SPEC } from '@labosfera/shared-spa/lib/journal/specs';
...
export interface ExperimentRefs {
  ...
  /** §21: legacy tbody (не используется). */
  journalBody: HTMLElement;
  /** §21: контейнер для shared `renderJournalTable` (с FRICTION_SPEC). */
...
export class FrictionExperiment {
  ...
  /** §20.4 — счётчик измерений, зафиксированных auto-режимом (от reset). */
  #autoRecordedCount = 0;
  /** §21: «черновик» введённых учеником derived-значений по timestamp. */
    ...
    if (transition.isSliding) {
      ...
      setTimeout(() => {
        if (this.#store.get().measurementStep === 'sliding') {
          ...
          this.#refreshUi();
          // §20.4 — auto-режим: фиксируем сразу.
  ...
  /**
   * §20.4 — auto-fire записи когда state в `ready-to-record` и режим=auto.
   * Защита от повтора: следующий auto-fire только после новой пары
   * pulling-static → sliding → ready.
   */
  ...
  #wireUp(): void {
    ...
    // §20.4 REFERENCE.md — toggle «manual / auto» (если слот в шаблоне).
    ...
    // В auto-режиме кнопка пишет сразу со state-значениями, без формы.
  ...
  /**
   * Оборачивает элемент в .attached-eq + добавляет detach (×) кнопку.
   * §20.3 REFERENCE.md — единый shared utility `attachDetachButton`.
   */
  ...
  #refreshRecordButton(): void {
    // §21 UX-v2: pending-кнопка «Записать» — только в semi-auto.
    // В fully-manual / fully-auto скрыта полностью.
  ...
  #refreshMeasurementPanel(): void {
    ...
    // §21: shared `renderJournalTable` с FRICTION_SPEC.
    ...
    const showEmptyManual =
      mode === 'fully-manual' && s.measurements.length === 0 && !!this.#attachedBlockEl;
    const hasDataOrEmpty = hasData || showEmptyManual;
    this.#refs.journalEmpty.hidden = hasDataOrEmpty;
    if (this.#refs.journalHost) {
      this.#refs.journalHost.hidden = !hasDataOrEmpty;
      ...
      renderJournalTable(this.#refs.journalHost, FRICTION_SPEC, rows, {
        mode,
        ...
        onVerify: (rowIdx) => {
          ...
          this.#journalDrafts.set(m.timestamp, draft);
          ...
          const verdicts = verifyRow(FRICTION_SPEC.columns, tempRow);
