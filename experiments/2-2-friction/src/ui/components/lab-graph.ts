...
/** Минимальная точка для отрисовки. Опыт сам решает, что подавать
 *  (force/extension для пружины, frictionForce/normalForce для трения и т.п.). */
interface Measurement {
  id: string;
  /** X-координата (например, удлинение в см или нормальная сила в Н). */
  extension: number;
  /** Y-координата (сила в Н). */
  force: number;
}

interface GraphData {
  measurements: ReadonlyArray<Measurement>;
  fitSlope: number | null; // наклон линейной регрессии (k или μ), null если данных <2
}
...
export class LabGraph extends HTMLElement {
  ...
  #drawPoint(m: Measurement, xToPx: (x: number) => number, yToPx: (f: number) => number): void {
    ...
    title.textContent = `F=${m.force.toFixed(2)}, x=${m.extension.toFixed(2)}`;
