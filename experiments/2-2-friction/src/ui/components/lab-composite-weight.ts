...
<svg class="rod" viewBox="0 0 60 130" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" hidden>
  ...
  <!-- Бирка с массой удалена по правилу «не подписывать массу» (ФИПИ-канон:
       ученик взвешивает на динамометре). -->
...
export class LabCompositeWeight extends HTMLElement {
  ...
  getWeightHookY(): number {
    ...
    // Диск — низ диска (на боковой поверхности нет крюка, диск надевается).
    // Возвращаем нижний край.
    return rect.height;
  }
  ...
  #renderDisc(): void {
    ...
    // Метка массы — на боковой поверхности
    // Подпись массы удалена по правилу «не подписывать массу».
