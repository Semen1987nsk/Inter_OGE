...
### 30.3. Спецификация Прил. 2 — опыты по комплектам №1 и №2
...
| 1.2 | Измерение архимедовой силы (цилиндры №2–4) | `kit-1-hydrostatics/src/screens/archimedes/` | ⚠️ **частично** | Только вода. Полное погружение. Нет систематического сбора F_арх(V) |
...
| 1.4 | Исследование F_арх(ρ_жидкости) (вода ↔ соляной раствор) | — | ❌ заглушка | Может быть встроено в archimedes/ через переключатель liquid |
| 1.5 | Независимость F_арх(m) при равном V (цилиндры №1 и №2) | — | ❌ заглушка | №1 сталь (195 г) + №2 алюм (70 г), оба V=25 см³ |

**Итого kit-1: 1 ✅ + 1 ⚠️ + 3 ❌ = 1/5 полностью + 1 частично.**
...
| **Измерение работы силы трения (A = F_тр · s)** | `kit-2-forces/src/screens/friction/` Task B | ❌ **«ложно-готовый»** | UI-задача показана, но `distanceMm`/`work` всегда `null` (FrictionExperiment.ts:286-287). Реализовать = собрать distanceMm + рассчитать work + добавить колонку в SPEC |
| **Измерение силы упругости (F_упр одной точкой)** | — | ❌ нет | Простейший: подвесить груз, отчёт динамометра. Можно как mini-режим в spring-stiffness/ или отдельный screen |
...
**Итого kit-2: 5 ✅ + 1 ❌ ложно-готовый + 1 ❌ отсутствует = 5/7 (если разделять Task A/B/C/D) или 3/6 (если считать friction монолитом).**
...
|---|---|---|---|---|---|
| 1 Гидростатика | 5 | 2 (плотность 1.1, F_А от V 1.3) | 1 (архимед 1.2 — только вода) | 2 (1.4, 1.5) | — |
| 2 Силы и пружины | 6 | 5 (жёсткость, μ, F_тр(N), F_тр(пов), Гука-график) | — | 2 (работа трения, F_упр) | 1 (работа упругости) |
| 3 Электр. цепи | планируется | 0 | — | — | — |
| 4 Оптика | планируется | 0 | — | — | — |
| 5 Колебания | планируется | 0 | — | — | — |
| 6 Рычаг | планируется | 0 | — | — | — |
| 7 Теплота | планируется | 0 | — | — | — |

**Целевое для MVP v0.4 (kit-1 + kit-2 — 100% ФИПИ):**
- kit-1: достроить 1.3, 1.4, 1.5 + добавить переключатель жидкости в archimedes
- kit-2: дореализовать `friction/` Task B (work = F_тр·s) + добавить
  «Измерение F_упр» (как mini-режим в spring-stiffness/ или отдельным экраном)
