...
// §20 REFERENCE.md — единый shared DragController вместо локального.
// Локальный `experiments/2-1-spring/src/controller/DragController.ts` остаётся
// до полной миграции kit-1 (в качестве back-compat для тестов), но активно
// больше не используется.
import {
  DragController,
  type SnapZone,
} from '@labosfera/shared-spa/controller/DragController';
import {
  attachDetachButton,
  injectDetachButtonStyles,
} from '@labosfera/shared-spa/ui/detach-button';
...
import type { JournalVerdict } from '@labosfera/shared-spa/lib/journal/types';

const RECORD_MODE_KIT = 'kit-2';
type AttachKind = 'spring' | 'dynamometer' | 'weight';
...
export interface ExperimentRefs {
  ...
  /** §21: legacy table (если есть в DOM, не используется). */
  journalTable: HTMLTableElement;
  /** §21: legacy tbody (не используется, оставлен для совместимости refs). */
  journalBody: HTMLElement;
  /** §21: контейнер для shared `renderJournalTable` (с SPRING_SPEC). */
  ...
  /** Форма ручного ввода в журнал (l₀, l₁, m). */
  ...
  /** §20.4 REFERENCE.md — слот для toggle «manual / auto» (опционально). */
...
export class SpringExperiment {
  ...
  /** §20.4 REFERENCE.md — disposer для record-mode toggle. */
  #detachRecordModeToggle: (() => void) | null = null;
  /** §20.4 — счётчик измерений, уже зафиксированных auto-режимом (от reset). */
  #autoRecordedCount = 0;
  /** §21: «черновик» введённых учеником derived-значений по measurement.id. */
  #journalDrafts = new Map<number, Record<string, number>>();
  /** §21: verdicts по measurement.id после клика ✓. */
  ...
  /** Cleanup: отключить record-mode toggle disposer. Вызывается перед unmount. */
  destroy(): void {
  ...
  reset(): void {
    ...
    // §20.4 — позволить auto-режиму снова фиксировать после reset.
    this.#autoRecordedCount = 0;
  ...
  #wireUp(): void {
    this.#refs.cards.forEach((card) => {
      ...
      this.#drag.attach(draggable, {
        ...
        onDragEnd: (accepted) => {
          ...
          this.#refreshDropZoneVisibility();
          // Defensive: убрать все «прилипшие» gold-rings (.snap-target) на случай
          // если onHover(false) от DragController не пришёл при resigning drop.
          // Без этого после нескольких drag'ов накапливаются пульсирующие кольца.
    ...
    // §20.4 REFERENCE.md — toggle «manual / auto» (если слот есть в шаблоне).
    ...
    // Кнопка «Записать в журнал» теперь ОТКРЫВАЕТ форму ручного ввода (не записывает сразу).
    // В auto-режиме (toggle) — пропускаем форму, пишем сразу со state-значениями.
  ...
  #highlightSnapTarget(active: boolean): void {
    // Перед добавлением подсветки — снять с ВСЕХ ранее подсвеченных,
  ...
  #mountInStack(element: HTMLElement): void {
    ...
    // §20.3 REFERENCE.md — единый `×` через shared utility.
    // Стили инжектируются один раз; повторный вызов — no-op.
    injectDetachButtonStyles();
    const equipmentId = element.dataset['equipmentId'] ?? 'оборудование';
    attachDetachButton(wrapper, {
      equipmentId,
      prettyName: this.#prettyEquipmentName(equipmentId),
      onDetach: () => this.#detachElement(element),
      // Element host (lab-spring-board, lab-dynamometer) сам обрабатывает
      // Delete/Backspace в составе своих keyboard-shortcuts — не дублируем.
  ...
  /**
   * Человекочитаемое имя оборудования для aria-label `×`-кнопки.
   * §20.3 REFERENCE.md — единый стандарт detach-кнопок.
   */
  #prettyEquipmentName(equipmentId: string): string {
    const map: Record<string, string> = {
      'spring-k50': 'Пружина k=50 Н/м',
      'spring-k10': 'Пружина k=10 Н/м',
      'dyno-1': 'Динамометр 1 Н',
      'dyno-5': 'Динамометр 5 Н',
      'w-100-1': 'Груз 100 г №1',
      'w-100-2': 'Груз 100 г №2',
      'w-100-3': 'Груз 100 г №3',
      rod: 'Стержень',
      'disc-10': 'Диск 10 г',
      'disc-20': 'Диск 20 г',
      'disc-50': 'Диск 50 г',
    };
    return map[equipmentId] ?? 'оборудование';
  }
  ...
  #updateChainPositions(): void {
    ...
    while (i < chain.length) {
      ...
      if (!wrapper?.classList.contains('attached-eq')) {
        i++;
        continue;
      }
      ...
      // Физика наборного груза: диски НЕ висят гирляндой на крючках —
      // они надеваются на штангу через центральное отверстие и лежат
      // СТОПКОЙ на нижнем фланце штанги (между нижним и верхним фланцами).
      // См. lab-composite-weight.ts: нижний фланец штанги в y=94 (vbox 60×130).
      // cursorY после штанги остаётся на её нижней петле — следующий за стопкой
      // элемент (если будет) висит на петле штанги обычным образом.
      if (
        el.tagName === 'LAB-COMPOSITE-WEIGHT' &&
        el.getAttribute('kind') === 'rod'
      ) {
        const rodHeight = el.getBoundingClientRect().height;
        const ROD_LOWER_FLANGE_VBY = 94; // y нижнего фланца в SVG-vbox штанги
        const ROD_VBOX_HEIGHT = 130;
        const lowerFlangeOffset = (ROD_LOWER_FLANGE_VBY / ROD_VBOX_HEIGHT) * rodHeight;
        // Стержень штанги должен быть визуально над дисками (диски надеты на него).
        wrapper.style.zIndex = '2';

        // Высота диска в SVG-vbox: эллипс верха в y=8 + height + эллипс низа.
        // В DOM это полная высота host'а (lab-composite-weight задаёт фикс. height
        // через :host([kind=disc][mass=X])).
        // Стопка растёт ВВЕРХ от нижнего фланца штанги: первый положенный диск
        // самый нижний, следующий — на нём.
        let stackBottomAbsolute = elTop + lowerFlangeOffset;
        let j = i + 1;
        while (
          j < chain.length &&
          chain[j]!.tagName === 'LAB-COMPOSITE-WEIGHT' &&
          chain[j]!.getAttribute('kind') === 'disc'
        ) {
          const disc = chain[j]!;
          const discWrapper = disc.parentElement;
          if (discWrapper?.classList.contains('attached-eq')) {
            const discHeight = disc.getBoundingClientRect().height;
            const discTop = stackBottomAbsolute - discHeight;
            discWrapper.style.top = `${discTop}px`;
            // Низ следующего диска лежит на ВЕРХНЕЙ ПЛОСКОСТИ этого диска
            // (sideTop = y=8 в vbox 60×30 → 8/30 * discHeight от верха host'а).
            const DISC_TOP_VBY = 8;
            const DISC_VBOX_HEIGHT = 30;
            stackBottomAbsolute =
              discTop + (DISC_TOP_VBY / DISC_VBOX_HEIGHT) * discHeight;
            // Z-order: диски ниже стержня штанги, но в порядке стопки —
            // верхние диски визуально перекрывают нижние боковые поверхности.
            discWrapper.style.zIndex = '1';
          }
          j++;
        }
        i = j;
  ...
  #handleScaleClick(valueMm: number): void {
    ...
    this.#refreshUi();
    // §20.4 — auto-режим: сразу пишем без формы.
    this.#maybeAutoRecord();
  }

  /** §20.4 REFERENCE.md — текущий режим записи. */
  ...
  #handleRecordModeChange = (mode: RecordMode): void => {
    if (mode === 'fully-auto') this.#maybeAutoRecord();
    this.#refreshUi();
  };

  /**
   * §21 — auto-fire записи когда state в `ready-to-record` и режим=fully-auto.
   * Защита от повтора через #autoRecordedCount: если зафиксировали,
   * следующий auto-fire будет только после новой пары кликов l₀/l₁.
   */
  #maybeAutoRecord(): void {
    if (this.#recordMode() !== 'fully-auto') return;
    const s = this.#store.get();
    if (s.measurementStep !== 'ready-to-record') return;
    if (s.measurements.length === this.#autoRecordedCount) {
      this.#recordMeasurement();
      this.#autoRecordedCount = s.measurements.length + 1;
    }
  }
  ...
  #refreshRecordButton(): void {
    const s = this.#store.get();
    // §21 UX-v2: pending-кнопка «Записать» — это семантика semi-auto.
    // В fully-manual ученик пишет ВСЁ руками; в fully-auto программа сама.
    const mode = getRecordMode(RECORD_MODE_KIT);
    if (mode !== 'semi-auto') {
      this.#refs.recordBtn.setAttribute('hidden', '');
  ...
  #renderJournal(): void {
    ...
    // даже без записанных measurement'ов — ученик пишет ВСЁ руками.
    // sentinel-timestamp = -1 (для UI-only empty row).
    ...
    if (hasData) {
      this.#refs.journalEmpty.setAttribute('hidden', '');
    } else {
      this.#refs.journalEmpty.removeAttribute('hidden');
    }
    ...
    const rows: import('@labosfera/shared-spa/lib/journal/types').JournalRow[] =
    ...
    renderJournalTable(this.#refs.journalHost, SPRING_SPEC, rows, {
      ...
      onCellInput: (rowIdx, key, value) => {
        ...
        // §21 UX-v2: pending empty manual row хранится под ts=-1.
      ...
      onVerify: (rowIdx) => {
        ...
        const dlMm = Math.round(m.extension * 10);
        const lAtLoad = l0 !== null ? l0 + dlMm : 0;
        const tempRow: import('@labosfera/shared-spa/lib/journal/types').JournalRow = {
          idx: rowIdx,
          timestamp: ts,
          values: {
            m_g: m.totalMass,
