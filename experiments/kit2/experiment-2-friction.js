...
class FrictionExperiment {
    ...
    placeEquipment(equipmentType, element) {
        ...
        if (equipmentType === 'block' && !this.state.blockPlaced) {
            if (!this.state.surfacePlaced) {
                this.showStatus('Сначала разместите направляющую!', 'warning');
                return;
            }
            this.state.blockPlaced = true;
            
            this.showStatus('Брусок размещён на направляющей!', 'success');
    ...
    onDragMove(event) {
        ...
        // Перемещаем САМ элемент (важно для interact.js dropzone)
        ...
        // Обновляем ghost чтобы следовал за элементом
        if (this.dragGhost) {
        ...
        // Сохраняем позицию для отрисовки зелёной зоны (в координатах canvas)
        const canvasRect = this.canvases.dynamic?.getBoundingClientRect();
        if (canvasRect && event.clientX && event.clientY) {
            ...
            // Перерисовываем для отображения зелёной зоны
            this.drawDynamic();
        }
    }
    ...
    switchToHorizontalMode() {
        ...
        if (this.state.weighingItems.length > 0) {
            this.showStatus('Сначала снимите грузы с динамометра!', 'warning');
            return;
        }
    ...
    drawBlockSnapZones(ctx) {
        ...
        // === SNAP ZONE НА ДИНАМОМЕТР (если размещён в вертикальном режиме) ===
        if (this.state.dynamometerPlaced && this.state.dynamometerMode === 'vertical') {
            const centerX = this.state.dynamometerX || 100;
            const anchorY = this.state.dynamometerY || 60;
